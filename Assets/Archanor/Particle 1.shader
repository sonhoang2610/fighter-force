...
////// Emissive:
				...
				float3 emissive = _MainTex_var.rgb*i.vertexColor.rgb*_TintColor.rgb*2;
                float3 finalColor = emissive;
                fixed4 finalRGBA = fixed4(finalColor,lerp( lerp( _MainTex_var.a*1.5f,0, 1-_TintColor.a),0, 1-i.vertexColor.a));
				// Softness factor
				float2 factor = (float2(1.0, 1.0) - abs(i.worldPos)) * _ClipArgs0;
				// Apply clipping
				//finalRGBA.a *= clamp(min(factor.x, factor.y), 0.0, 1.0);
                return finalRGBA;
            }
            ENDCG
        }
