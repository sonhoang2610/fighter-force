...
{	
	...
	public static class MMDebug 
	{
        ...
        public static RaycastHit2D RayCast(Vector2 rayOriginPoint, Vector2 rayDirection, float rayDistance, LayerMask mask,GameObject ignoreObject, Color color, bool drawGizmo = false)
        {
            if (drawGizmo)
            {
                Debug.DrawRay(rayOriginPoint, rayDirection * rayDistance, color);
            }
            RaycastHit2D[] hits = Physics2D.RaycastAll(rayOriginPoint, rayDirection, rayDistance, mask);
