...
////// Lighting:
////// Emissive:
                float4 node_765 = tex2D(_MainTex,TRANSFORM_TEX(i.uv0, _MainTex));
                float3 emissive = (node_765.rgb*(_Color.rgb*_Emissin));
                ...
                float node_2244 = (1.0 - i.uv0.g);
                return fixed4(finalColor,((saturate((node_2917+Fill))*node_765.a*step((_Amount*1.01),floor((1.0 - saturate(max(saturate(abs((1.0 - i.uv0.r)-i.uv0.r)),saturate(abs(i.uv0.g-node_2244))))) * _Density) / (_Density - 1)))+(node_2917*node_765.a*_MosaicStrength)));
            }
            ENDCG
        }
    }
