		...
		pass {   
			...
			*/
			...
			float4 frag (v2f i) : COLOR {
				...
				uv_dummy1 = uv_dummy1+fixed2(-5,0)*(_Time.y);    
				...
				uv_dummy1 = uv_dummy1+center_dummy1;    
				float2 uv_dummy1orgin = uv_dummy1;
				uv_dummy1 = float2(uv_dummy1.x >0 ?fmod(uv_dummy1.x,1+0) : (1+0) - fmod(abs(uv_dummy1.x),1+0), uv_dummy1.y >0 ?fmod(uv_dummy1.y,1+0): (1+0) - fmod(abs(uv_dummy1.y),1+0));
				bool discard_dummy1 = false;
				if(uv_dummy1.x>1 || uv_dummy1.y>1)
					discard_dummy1 = true;
				float4 rect_dummy1 =  float4(1,1,1,1);
				float4 color_dummy1 = tex2D(_MainTex,uv_dummy1);    
				if(discard_dummy1 == true) color_dummy1 = float4(0,0,0,0);
