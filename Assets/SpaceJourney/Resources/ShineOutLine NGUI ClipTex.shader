...
Shader "Hidden/Shader Weaver/ShineOutLine (TextureClip)"{   
	Properties {   
		...
		pass {   
			...
			*/
			...
			v2f vert (appdata_t IN) {
				...
				OUT.clipUV = (IN.vertex.xy * _ClipRange0.zw + _ClipRange0.xy) * 0.5 + float2(0.5, 0.5);
				return OUT;
			}   
			float4 frag (v2f i) : COLOR {
				...
				result.a *= tex2D(_ClipTex, i.clipUV).a;
				clip(result.a - 0);
				return result;
			}  
			ENDCG
