            ...
            if (isInit)
            {
                ...
                for (int i = 0; i < GameDatabase.Instance.LevelConfigScene.Length; ++i)
                {
                    if (GameDatabase.Instance.LevelConfigScene[i].requrireStarToUnlock > 0)
                    {
                        ...
                        if (pLevel.isLocked && GameManager.Instance.CurrentLevelUnlock >= pLevel.level-1 && GameManager.Instance.Database.getComonItem("Star").Quantity >= GameDatabase.Instance.LevelConfigScene[i].requrireStarToUnlock)
                        {
