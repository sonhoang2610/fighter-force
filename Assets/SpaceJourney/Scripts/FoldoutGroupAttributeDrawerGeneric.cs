...
namespace Sirenix.OdinInspector.Editor.Drawers
{
    ...
    public class FoldoutGroupAttributeDrawerGeneric<T> : OdinGroupDrawer<T> where T : ExpanedListEvent
    {
        protected LocalPersistentContext<bool> IsVisible;
        private StringMemberHelper TitleHelper;
        private float t;

        public virtual void changedGroup(bool pBool)
        {
        ...
        protected override void DrawPropertyLayout(GUIContent label)
        {
            ...
            bool pValue = SirenixEditorGUI.Foldout(this.IsVisible.Value, content);
            if(pValue  != this.IsVisible.Value)
            {
                changedGroup(pValue);
                this.IsVisible.Value = pValue;
            }
